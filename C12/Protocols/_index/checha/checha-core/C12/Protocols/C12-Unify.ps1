...
# --- консольне кодування, щоб у логах не було "кракозябр"
...
function Move-ItemSafe([string]$src, [string]$dst){
  ...
  if (Test-Path -LiteralPath $dst) {
    ...
    Write-Warn "Ціль існує, збережу як: $alt"
    $dst  = $alt
  }
...
# --- 3) README у ключових папках (без ком та форматних рядків)
if ($CreateReadme) {
  $keyDirs = @(
    $RootFull
